...
const loginUser = catchAsync(async (req, res) => {
  ...
  const user = await User.findOne({ email }).select("+role -refreshToken +password +isVerified");
...
export default { refreshAccessToken, registerUser, loginUser, loginWithOtp, verifyLoginOtp, handleSocialLogin, sendForgotPassOtp, verifyForgotPassOtp, resetPassword ,sendRegisterOtp,verifysendRegisterOtp };
