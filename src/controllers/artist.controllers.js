...
const uploadArtDocuments = catchAsync(async (req, res) => {
  ...
  const updatedProfile = await ArtistProfile.findByIdAndUpdate(
    req.user._id,
    { $push: { "artInfo.artDocuments": { $each: uploadedArtDocuments } } },
    { new: true }
  );

  res.status(200).json({ message: "Art documents uploaded successfully", profile: updatedProfile });
});
...
const uploadPerformanceDocuments = catchAsync(async (req, res) => {
  ...
  const updatedProfile = await ArtistProfile.findByIdAndUpdate(
    req.user._id,
    { $push: { "performanceInfo.performanceDocuments": { $each: uploadedPerformanceDocuments } } },
    { new: true }
  );

  res.status(200).json({ message: "Performance documents uploaded successfully", profile: updatedProfile });
});
...
const uploadAwardDocuments = catchAsync(async (req, res) => {
  ...
  const updatedProfile = await ArtistProfile.findByIdAndUpdate(
    req.user._id,
    { $push: { "awardsInfo.awardDocuments": { $each: uploadedAwardDocuments } } },
    { new: true }
  );
